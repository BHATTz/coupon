...
@section('content')
    <form id="couponForm" action="{{ route('admin.update-store',$store->id) }}" method="post" enctype="multipart/form-data">
        @csrf
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <input name="name" type="text" class="form-control"
                                                placeholder="Stores Name" value={{ $store->name }}>
                                            @error('name')
                                    ...
                                    <div class="col-sm-6">
                                        <div class="form-group">
                                            <input name="website_url" type="url" class="form-control"
                                                placeholder="Website URL" value={{ $store->website_url }}>
                                            @error('website_url')
                                                <div class="alert alert-danger">{{ $message }}</div>
                                            @enderror
                                        </div>
                                ...
                                <div class="row clearfix">
                                    <div class="col-sm-8">
                                        <div class="radio" style="display: inline-block; margin-right: 20px;">
                                            <input type="radio" name="is_hidden" id="hide-radio" value="1" {{ $store->is_hidden == 1 ?"checked":"" }}>
                                            <label for="hide-radio">Hide</label>
                                        </div>
                                        <div class="radio" style="display: inline-block;">
                                            <input type="radio" name="is_hidden" id="show-radio" value="0" {{ $store->is_hidden == 0 ?"checked":"" }}>
